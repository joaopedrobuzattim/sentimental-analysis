import requests
from ...data_access.most_forks_repositories import ForksRepositories
from src.utils import constants

def extractMostForks(token):
  ...
  for i in range(1, 11):
    ...
    for item in response["items"]:
      with ForksRepositories() as forksRepositories:
        forksRepositories.create(
          item["owner"]["login"],
          item["name"],
          item["id"],
